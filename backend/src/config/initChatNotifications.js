...
export const initializeChatNotificationsTables = async () => {
  await pool.query(`
    CREATE TABLE IF NOT EXISTS flix.chat_messages (
      ...
      sender_id INTEGER NOT NULL REFERENCES flix.users(id_user) ON DELETE CASCADE,
      receiver_id INTEGER NOT NULL REFERENCES flix.users(id_user) ON DELETE CASCADE,
